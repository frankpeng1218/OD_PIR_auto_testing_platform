...
class WiFiManager:
    def __init__(self, ssid, password, static_ip=None):
        self.ssid = ssid
    ...
    async def connect_to_wifi(self):
        ...
        if self.static_ip:
            self.wlan.ifconfig(self.static_ip)  # 设置静态IP
...
# 使用示例
wifi_manager = WiFiManager('dlink-Sales', '1234567890', static_ip=('192.168.1.100', '255.255.255.0', '192.168.1.1', '8.8.8.8'))
asyncio.run(wifi_manager.connect_to_wifi())  # 连接WiFi并检查状态
